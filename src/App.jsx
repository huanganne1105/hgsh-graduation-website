...
function pickResult(answers) {
  const scores = Object.keys(quiz.results).reduce((acc, key) => ({ ...acc, [key]: 0 }), {});
  ...
  return { key, scores, result: quiz.results[key] };
}
  ...
  if (page === "home") {
    return (
      <main className="phone home-page">
        ...
        <button className="start-hitbox" onClick={() => setPage("quiz")} aria-label="回到高一" />
      </main>
    );
  }
  ...
  if (!done) {
    ...
    return (
      <main className="phone quiz-page">
        <img className="bg" src={current % 2 === 0 ? ASSETS.bg1 : ASSETS.bg2} alt="" />
        <motion.section
          key={current}
        ...
        >
          <div className="progress">{current + 1} / {quiz.questions.length}</div>
          <h1>
          ...
          <div className="options">
            {q.options.map((option, index) => (
              <button key={index} onClick={() => choose(option.type, option.text)}>
                {option.text}
              </button>
            ))}
          </div>
      ...
      >
        {final.key === "unicorn" ? (
          <div className="result-images">
            <img className="result-image" src="/images/unicorn.png" />
